...
struct MultiSegmentVectorTests {
	/// E.3 exercises the corrected ChaCha20-Poly1305 code point (0x001D).
	@Test func chaCha20SingleSegmentMatchesE3() throws {
		let v = try Vectors.load("E3")
	...
	/// E.7 (two segments): each segment decrypts and re-encrypts exactly, and segments
	/// can be processed in any order (random access).
	@Test func twoSegmentsMatchE7InAnyOrder() throws {
	...
	/// (E.7 uses epoch_length=1, so segments 0 and 1 share epoch 0).
