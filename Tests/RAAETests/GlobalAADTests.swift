...
/// draft-sullivan-cfrg-raae-02; the empty default is pinned against Appendix F.1 and
/// non-empty `G` against Appendix F.2 (see `Spec/NOTES.md`).
@Suite("Global associated data (G)")
struct GlobalAADTests {
	/// draft-02 Appendix F.2, "G = raae-demo-g": the F.1 schedule (same CEK, salt,
	...
	static let f2CommitmentHex =
		"d8eedb1fa0f77428cc33d252eb307796ae3bb911c2f6ea7a9e5b0bde312afd73"

	func f1Inputs() throws -> (cek: [UInt8], info: PayloadInfo, commitment: [UInt8]) {
	...
	@Test func emptyGMatchesVendoredF1Commitment() throws {
		// The empty default is framed as a zero-length element (the published-draft
		// convention), which is exactly the commitment the vendored F.1 corpus
		// carries — both spelled and defaulted derive it.
		let (cek, info, commitment) = try f1Inputs()
		...
		// Pin the vendored value directly against draft-sullivan-cfrg-raae-02 Appendix
		// F.1 so the vendored JSON and the published draft cannot silently drift apart.
		#expect(
	...
	/// Pin the remaining vendored empty-G commitments to their published draft-02
	/// literal values (identical in -01, which introduced them). F.1 is pinned above;
	/// these complete the corpus so no vendored commitment rests on JSON↔derivation
	/// self-consistency alone. Each literal was cross-verified against an independent
	/// from-scratch implementation of the §4.3 labeled KDF, which reproduces both the
	/// retired pre-G E.1 value `020e115b…` and the always-framed-empty-G `47ea0ec7…`
	/// exactly. F.9 shares F.1's CEK + payload_info, so it must derive the same value.
	@Test func emptyGCommitmentsPinnedToPublishedDraft() throws {
		let expected: [(String, String)] = [
			("F5", "9ef7166bbce42787fd834f79d29f85b66a050b24f372ecfb79a66b3f2fdc1acb"),
			("F9", "47ea0ec7409b9b95d676019917a19f1c5831eb236aba459063458e525d130d0c"),
			("F16", "9285553e10209c27bb5858b621426513b0832f26d7ee813d9dd62c218ce6972a"),
			("F17", "bf20f8c7691934f0ccf767b2a5ac19e467228674414f68d839a6698a3edd1813"),
		]
		for (name, hex) in expected {
			let v = try Vectors.load(name)
			// Derived value matches the independently-verified published literal …
	...
	@Test func nonEmptyGPinsDraftF2Vector() throws {
		// Byte-exact against draft-sullivan-cfrg-raae-02 Appendix F.2.
		let (cek, info, _) = try f1Inputs()
		let schedule = try PayloadSchedule(
			protocolID: ProtocolID.mutable, cek: cek, payloadInfo: info,
			globalAAD: Self.demoG)
		#expect(Hex.encode(schedule.commitment) == Self.f2CommitmentHex)
	}
